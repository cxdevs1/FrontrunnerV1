...
export default function Home() {
  ...
  const generateSnapshotReport = (result: AnalysisResult) => {
    ...
    const eventType = isMigration ? "MIGRATION" : "ADDITION";
    const indexDisplay = result.indexTarget.replace("SP", "");
    const fromIndexDisplay = result.fromIndex?.replace("SP", "") || "";
    
    let report = `=========================================
      INDEX SNAPSHOT: ${result.ticker}
=========================================
ACTION:    ${result.action}
INTENSITY: ${result.intensity}
-----------------------------------------
EVENT TYPE: ${eventType}
TARGET:     S&P ${indexDisplay}
`;

    if (isMigration) {
      report += `EXITING:    S&P ${fromIndexDisplay}
`;
    }

    report += `
MECHANICAL DATA:
  - Pressure Score:  ${result.pressureScoreDisplay}
  - Est. Net Demand: ${result.requiredSharesDisplay} Shares
  - Algo Activity:   ${result.isAlgoActive ? "CAUTION" : "CLEAR"}
`;

    if (isMigration && result.buyPressure !== undefined && result.sellPressure !== undefined) {
      report += `
  - Buying Demand:   ${result.buyPressure.toLocaleString()} shares
  - Selling Pressure: ${result.sellPressure.toLocaleString()} shares
`;
    }

    report += `-----------------------------------------
Report generated by FrontrunnerV1
Institutional-Grade Index Intelligence
=========================================`;
  ...
  const handleCopyToClipboard = async () => {
    ...
    const text = generateSnapshotReport(analysisResult);
    await navigator.clipboard.writeText(text);
    setCopied(true);
    toast({ title: "Copied to clipboard", description: "Snapshot report ready to paste" });
    setTimeout(() => {
      setCopied(false);
  ...
  const handleDownloadReport = () => {
    ...
    a.href = url;
    a.download = `${analysisResult.ticker}_snapshot_${new Date().toISOString().split('T')[0]}.txt`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    toast({ title: "Report downloaded", description: `${analysisResult.ticker} snapshot saved` });
    setShowShareMenu(false);
  };
                ...
                <div className="pt-4 mt-4 border-t border-slate-200 dark:border-slate-700">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 px-3 py-2 rounded-lg bg-amber-100 text-amber-700 border border-amber-300 dark:bg-amber-900 dark:text-amber-300 dark:border-amber-700 font-semibold flex items-center justify-center gap-2 hover:shadow-md transition-all"
                    ...
                    <div className="relative">
                      <button
                        ...
                        className="px-3 py-2 rounded-lg bg-indigo-600 text-white border border-indigo-700 font-semibold flex items-center gap-2 hover:bg-indigo-700 hover:shadow-md transition-all"
                      ...
                      {showShareMenu && (
                        <div className="absolute right-0 bottom-full mb-2 w-48 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 shadow-lg overflow-hidden z-10">
                          <button
                          ...
                          >
                            ...
                            <span className="font-medium text-slate-700 dark:text-slate-200">Download Report</span>
                          </button>
                        </div>
                      )}
                    </div>
